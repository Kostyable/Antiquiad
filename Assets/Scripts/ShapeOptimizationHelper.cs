...
public static class ShapeOptimizationHelper {
	...
	private static void DouglasPeuckerReductionRecursive(List<Vector2> points, Int32 firstPoint, Int32 lastPoint,
		Double tolerance, ref List<Int32> pointIndexsToKeep)
	{
		...
		if (maxDistance > tolerance && indexFarthest != 0)
		{
			...
			DouglasPeuckerReductionRecursive(points, firstPoint, indexFarthest, tolerance,
				ref pointIndexsToKeep);
			DouglasPeuckerReductionRecursive(points, indexFarthest, lastPoint, tolerance,
				ref pointIndexsToKeep);
		}
	}
	...
	{
		double area = Math.Abs(.5f * (Point1.x * Point2.y + Point2.x * Point.y + Point.x * Point1.y - Point2.x
			* Point1.y - Point.x * Point2.y - Point1.x * Point.y));
