...
public class Map : MonoBehaviour
{
    ...
    [SerializeField] private int riversCost;
    ...
    private void Awake()
    {
        ...
        MinX = _cells[0, 0].GetComponent<Transform>().position.x - 1.5f;
        MaxY = _cells[0, 0].GetComponent<Transform>().position.y + 1.5f;
        MaxX = _cells[size.y - 1, size.x - 1].GetComponent<Transform>().position.x + 1.75f;
        MinY = _cells[size.y - 1, size.x - 1].GetComponent<Transform>().position.y - 1.85f;
        ...
        GameLogic.RiverCost = riversCost;
    ...
    // private void ColorPath(List<Cell> path)
    // {
    //     foreach (var cell in path)
    //     {
    //         // Предполагаем, что у ячейки есть компонент SpriteRenderer
